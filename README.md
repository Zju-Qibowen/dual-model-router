...
DeepSeek v4 不支持多模态输入（图片）。当任务包含图片时，路由会自动：

1. **将图片交给强模型解析**为文本描述（文字转录、结构分析、视觉元素描述）
2. **将文本描述合并到原始任务中**
3. **继续正常路由** — 简单任务仍走弱模型（现在有了图片的文字描述），复杂任务走强模型

这样图片理解只消耗强模型的一次调用，后续文本处理仍可享受弱模型的低成本优势。而不是粗暴地"有图片就全部走强模型"。
...
| 工具 | 说明 |
|------|------|
| `route_and_answer` | 自动路由并执行任务（主入口），支持 `images_json` 参数传入图片 |
| `ask_weak` | 直接调用 DeepSeek，图片会自动预处理为文本描述 |
